        ...
        <div className="max-w-6xl mx-auto px-6">
          <ScrollReveal>
            <a
              ...
              className="card block p-8 md:p-10 group relative overflow-hidden"
            >
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="font-mono text-[10px] tracking-[2px] uppercase" style={{ color: "#059669" }}>AI PRODUCT · 2026 · LIVE</span>
                  </div>
                  <h2 className="font-display text-2xl md:text-3xl font-bold mb-2 group-hover:translate-x-1 transition-transform" style={{ color: "var(--text-primary)" }}>Library Forge</h2>
                  <p className="text-sm leading-relaxed max-w-lg" style={{ color: "var(--text-muted)" }}>
                    AI-powered design system platform enabling teams to generate component libraries and documentation automatically.
                  </p>
                </div>
                <div className="flex items-center gap-2 font-mono text-sm font-medium group-hover:gap-3 transition-all" style={{ color: "#059669" }}>
                  Visit Live App
            ...
            {projects.map((p) => (
              <StaggerItem key={p.title} className={p.featured ? "md:col-span-2" : ""}>
                <div className="card p-8 h-full">
                  <span className="inline-block font-mono text-[10px] tracking-[2px] uppercase px-3 py-1 rounded-md mb-5" style={{ color: p.badgeColor, background: `${p.badgeColor}10`, border: `1px solid ${p.badgeColor}20` }}>
                    {p.badge}
                  </span>
                  <h3 className="font-display text-xl md:text-2xl font-semibold mb-3" style={{ color: "var(--text-primary)" }}>{p.title}</h3>
                  <p className="text-sm leading-relaxed mb-5" style={{ color: "var(--text-muted)" }}>{p.desc}</p>
                  <div className="flex flex-wrap gap-2">
                    {p.metrics.map((m) => (
                      <span key={m} className="font-mono text-[10px] px-3 py-1.5 rounded-lg" style={{ background: "var(--bg-tertiary)", border: "1px solid var(--border)", color: "var(--text-muted)" }}>{m}</span>
                    ))}
                  </div>
                </div>
              </StaggerItem>
